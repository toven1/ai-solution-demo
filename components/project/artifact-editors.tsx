...
export function PersonaEditor({ projectId, personas }: { projectId: string; personas: any[] }) {
  ...
  async function generate() {
    ...
    setToast(res.ok ? { type: "success", message: "페르소나 3개를 생성했습니다." } : { type: "error", message: data?.error ?? "생성에 실패했습니다." });
    router.refresh();
  }
  async function save() {
    setBusy(true);
...
export function BMCanvasEditor({ projectId, canvas }: { projectId: string; canvas: any | null }) {
  ...
  async function generate() { setBusy(true); const res = await fetch(`/api/projects/${projectId}/canvas/generate`, { method: "POST" }); setBusy(false); const data = res.ok ? null : await res.json().catch(() => null); setToast(res.ok ? { type: "success", message: "BM Canvas를 생성했습니다." } : { type: "error", message: data?.error ?? "생성에 실패했습니다." }); router.refresh(); }
...
export function MVPFeatureEditor({ projectId, features }: { projectId: string; features: any[] }) {
  ...
  async function generate(){setBusy(true); const res=await fetch(`/api/projects/${projectId}/mvp/generate`,{method:"POST"}); setBusy(false); const data=res.ok?null:await res.json().catch(()=>null); setToast(res.ok?{type:"success",message:"MVP 기능 후보를 생성했습니다."}:{type:"error",message:data?.error??"생성에 실패했습니다."}); router.refresh();}
...
export function ValidationExperimentEditor({ projectId, experiments }: { projectId: string; experiments: any[] }) {
  ...
  async function generate(){setBusy(true); const res=await fetch(`/api/projects/${projectId}/experiments/generate`,{method:"POST"}); setBusy(false); const data=res.ok?null:await res.json().catch(()=>null); setToast(res.ok?{type:"success",message:"검증 실험을 생성했습니다."}:{type:"error",message:data?.error??"생성에 실패했습니다."}); router.refresh();}
